...
def load_local_monthly_history() -> pd.DataFrame:
    sql = """
    SELECT
        user_id,
        category_id,
    ...
    ORDER BY user_id, category_name, year_month
    """
...
def build_training_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (user_id, category_name), g in df.groupby(["user_id", "category_name"]):
...
def replace_local_training_rows(df: pd.DataFrame) -> None:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM local_forecast_training_rows;")

            insert_sql = """
            INSERT INTO local_forecast_training_rows (
                user_id,
                category_id,
                category_name,
                year_month,
...
def main() -> None:
    df = load_local_monthly_history()
    if df.empty:
        print("No local monthly history found")
        return

    df = add_time_features(df)
    rows = build_training_rows(df)
    replace_local_training_rows(rows)
    print(f"Built {len(rows)} local personalized training rows")
