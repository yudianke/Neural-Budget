...
_use_fallback = False
_fallback_mode = None
# offline eval results — populated at load/reload time
...
def load():
    ...
    Offline eval gate:
      - After loading model artifacts, runs held-out evaluation before
        activating the model. If eval gate fails, refuses to activate
        and keeps the previous model (or enters degraded mode).
      - Gate is fail-open for infra issues (eval data unavailable).
      - Gate is fail-closed for quality issues (accuracy below threshold).
    """
    ...
    try:
        ...
        # Run held-out eval before activating the model. If gate fails,
        # keep previous model loaded (or enter degraded mode).
        ...
        if not eval_results.get("gate_passed", True):
            ...
            # If we have a previous working model, keep it
            ...
            # No previous model — enter degraded mode
            print(
                f"[M1] EVAL GATE FAILED for v{new_version}: {reason} "
        ...
        # Gate passed — activate the new model
...
def get_eval_results() -> dict:
    """Return the last offline evaluation results.

    Called by GET /metrics/evaluation endpoint.
    """
...
def rerun_eval() -> dict:
    ...
    Called by POST /admin/run-eval endpoint. Does NOT gate — just returns results.
    """
...
def reload() -> None:
    ...
    Includes offline eval gate — if the new model fails evaluation,
    the previous model is kept active (see load() for details).
    """
    global _booster, _label_encoder, _tfidf, _metadata, _model_version
    global _pipeline, _use_fallback, _fallback_mode, _last_eval_results, _last_run_id
