...
Uses httpx TestClient with mocked real_model to avoid MLflow dependency.
"""
...
# We need to mock real_model BEFORE importing server, since server imports it at module level.
# Use a fixture-based approach with importlib.

from schemas import M1Output, CategorySuggestion
...
def app_client(mock_real_model):
    ...
    with patch.dict("sys.modules", {"real_model": mock_real_model}):
        # Also mock offline_eval to avoid import issues
...
class TestPredictEndpoint:
    ...
    def test_predict_missing_field_returns_422(self, app_client):
        incomplete = {"transaction_id": "t1"}
        resp = app_client.post("/predict/category", json=incomplete)
