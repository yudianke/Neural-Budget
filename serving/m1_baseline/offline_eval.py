...
def _compute_eval_results(
    ...
    gate_reason = (
        f"macro_f1={macro_f1:.4f} >= {gate_floor}"
        if gate_passed
...
def run_sanity_eval(
    ...
    y_pred_list = []
    skipped = 0
    for i, (_, expected_cat, _) in enumerate(SANITY_CASES):
    ...
    result = _compute_eval_results(
        y_true, y_pred, label_encoder, "sanity", SANITY_GATE_FLOOR, model_version, start
    )
    if skipped:
...
def run_synthetic_eval(
    ...
    return _compute_eval_results(
        y_true, y_pred, label_encoder, "synthetic", SYNTHETIC_GATE_FLOOR, model_version, start
    )
...
def download_eval_data(run_id: str, tracking_uri: str) -> Optional[pd.DataFrame]:
    try:
        ...
        artifact_path = mlflow.artifacts.download_artifacts(run_id=run_id, artifact_path="eval_data")
